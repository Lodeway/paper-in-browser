...
//        System.out.println("DONE");
        ...
        Stream<StackFrame> stackStream = IntStream.range(2, stackTrace.length).mapToObj(i -> {
            ...
            // Correlating getStackTrace() with getClassContext() by index assumes both arrays describe identical frames.
            // That is not guaranteed on every JVM (CheerpJ reports reflection frames differently), so verify the
            // correlated class by name and otherwise resolve the frame's class from its name.
            String frameClassName = stackTrace[i].getClassName();
            int refIndex = i + classRefOffset.get();
            Class<?> ref = refIndex >= 0 && refIndex < classRef.length ? classRef[refIndex] : null;
            if (ref == null || !ref.getName().equals(frameClassName)) {
                ref = resolveFrameClass(frameClassName, ref);
            }
            frame.classRef = ref;
    ...
    private static Class<?> resolveFrameClass(String name, Class<?> fallback) {
        ClassLoader[] loaders = { Thread.currentThread().getContextClassLoader(), J_L_StackWalker.class.getClassLoader(), ClassLoader.getSystemClassLoader() };
